...
export default function IncomeManager() {
  ...
  const fetchData = async () => {
    try {
      const [incomeData, catData] = await Promise.all([
  ...
  const openModal = (income?: Income) => {
    if (income) {
      ...
      setCurrency(income.currency || 'PEN');
      setDescription(income.description || '');
      setCategoryId(income.category || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = incomeSchema.safeParse(dataObj);
    ...
    try {
      if (editingIncome) {
        await api.put(`/income/${editingIncome.id}`, result.data);
      } else {
        await api.post('/income', result.data);
      }
      await fetchData();
      closeModal();
    } catch (e) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-sm text-gray-500">Registra y administra tus ingresos</p>
        </div>
        <button
          onClick={() => openModal()}
                  ...
                  <tr key={inc.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                        inc.type === 'SALARY' ? 'bg-blue-50 text-blue-700' : 'bg-emerald-50 text-emerald-700'
                      }`}>
                        {inc.type === 'SALARY' ? t('salary') : t('additional')}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-gray-500">{getCategoryName(inc.category || '')}</td>
                    <td className="px-6 py-4 text-right font-bold text-gray-800">
                      {inc.currency === 'USD' ? '$' : 'S/'} {inc.amount.toFixed(2)}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => openModal(inc)} className="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors">
                          <Edit2 size={16} />
                        </button>
                        <button onClick={() => handleDelete(inc.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
              ...
              <form id="income-form" onSubmit={handleSubmit} className="space-y-5">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      value={type}
                      onChange={(e) => setType(e.target.value as 'SALARY' | 'ADDITIONAL')}
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors"
                    placeholder="Ej. Sueldo de la empresa"
                  />
                </div>
